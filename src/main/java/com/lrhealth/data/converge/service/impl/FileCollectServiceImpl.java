...
public class FileCollectServiceImpl implements FileCollectService {
    ...
    private void initialRegistry() {
        ...
        for (ConvTunnel convTunnel : queueTunnels) {
            ...
            if (tunnelMessage.getFileCollectInfoDto() != null
                    && tunnelMessage.getFileCollectInfoDto().getFileStorageMode() == 1) {
    ...
    private void startConsumer(String broker, String topic, String topicKey) {
        KafkaConsumer<Object, Object> consumer = dynamicConsumerFactory.createConsumer(topic, FILE_INTERFACE_GROUP_ID,
                broker);
    ...
    public void fileDataHandler(String topicKey, List<String> values) {
        ...
        AsyncFactory.convTaskLog(task.getId(),
                String.format("%s | table: %s获取记录%s条: ", DateUtil.now(), table, values.size()));
        ...
        try {
            DbOperateUtils.batchInsert(table, collect, JSON.parseArray(values.toString().toLowerCase()), conn,
                    datasourceConfig.getDbType().toLowerCase());
            AsyncFactory.convTaskLog(task.getId(),
                    String.format("%s | 文件采集方式： | table: %s插入记录%s条: ", DateUtil.now(), table, values.size()));
        } catch (Exception e) {
