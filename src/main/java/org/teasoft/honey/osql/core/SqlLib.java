...
import org.teasoft.honey.sharding.engine.*;
...
//				else
	...
	private static final Pattern UPSERT_SQL_SPILT_PATTERN = Pattern.compile("(?=\\bON\\b\\s+(?:DUPLICATE\\s+KEY\\s+UPDATE|CONFLICT\\s*\\(.+?\\)\\s*DO\\s+UPDATE\\s+SET))", Pattern.CASE_INSENSITIVE | Pattern.DOTALL);

	// 拼接upsert批量SQL有问题，这里修改一下  by baiyu
	private String[] getBatchExeSql(String sql0, int size, String placeholderValue) {
		StringBuilder batchSql = new StringBuilder(sql0);
		StringBuilder batchSql_forPrint = new StringBuilder(sql0);
		String[] batchExeSql = new String[2];

		String[] sql_split = UPSERT_SQL_SPILT_PATTERN.split(sql0);
		String sqlSuffix = "";
		if (sql_split.length == 2) {
			batchSql = new StringBuilder(sql_split[0]);
			batchSql_forPrint = new StringBuilder(sql_split[0]);
			sqlSuffix = sql_split[1];
		}
		...
		batchExeSql[0] = batchSql.append(sqlSuffix).toString();
		batchExeSql[1] = batchSql_forPrint.append(sqlSuffix).toString(); //for print log
